...
class ConfigManager{
	public:
		...
		bool GetDoubleArgument(std::string argName, double *result){		//変数を取得したときはtrue、そうでないときはfalse
			if(d_args.size() >= 1){
				for(int i=0;i<d_args.size();i++){				//配列の中を探す
				...
				for(int i=0;i<d_args.size();i++){				//配列の中を探す
					if(argName.compare(d_args[i].arg) == 0){	//d_args[i]のargとargNameが一緒＝入力された引数を見つけた
						if(d_args[i].read){						//この引数がすでにファイルから読み込まれていれば
							...
							for(int j=0;j<result.size();j++)result[j] = d_args[i].value[j];
							return true;							//trueを返して終了
						}	
						else{
							std::cerr << MTM_ERROR << "[ConfigManager]Cannot Find Argument \"" << argName << "\" From Configure File!" << MTM_FINISH;
				...
				for(int i=0;i<i_args.size();i++){
					if(argName.compare(i_args[i].arg) == 0){
						if(i_args[i].read){
							...
							for(int j=0;j<result.size();j++)result[j] = i_args[i].value[j];
							return true;
						}	
						else{
							std::cerr << MTM_ERROR << "[ConfigManager]Cannot Find Argument \"" << argName << "\" From Configure File!" << MTM_FINISH;
				...
				for(int i=0;i<s_args.size();i++){
					if(argName.compare(s_args[i].arg) == 0){
						if(s_args[i].read){
							...
							for(int j=0;j<result.size();j++)result[j] = s_args[i].value[j];
							return true;
						}	
						else{
							std::cerr << MTM_ERROR << "[ConfigManager]Cannot Find Argument \"" << argName << "\" From Configure File!" << MTM_FINISH;
				...
				if(line.find("#",0) != 0){										//行の頭に # があったらコメントアウト（無視）
					if(d_args.size() >= 1){
						for(int i=0;i<d_args.size();i++){
							...
							if(ret != std::string::npos){						//読み取った文字列からargを発見した場合
		...
		bool OutputSampleFile(const char *filename){
			...
			if(d_args.size() >= 1 || i_args.size() >= 1 || s_args.size() >= 1){
				...
				if(d_args.size() >= 1){		//全部0にしたサンプルを書き込む
					for(int i=0;i<d_args.size();i++)ofs << d_args[i].arg << " 0.0000" << std::endl;
				}
				if(i_args.size() >= 1){
					for(int i=0;i<i_args.size();i++)ofs << i_args[i].arg << " 0" << std::endl;
				}
				if(s_args.size() >= 1){
					for(int i=0;i<s_args.size();i++)ofs << s_args[i].arg << " [string data]" << std::endl;
				}
