...
#include <ssm.hpp>
#include "ssm-observation-probability-map.hpp"
...
int main(int argc, char *argv[], char **env) {
	...
	{ // ---> initialize
		...
		if( !::is_proc_shutoff() ){
			SSMObservProbMap		ssm_map;		// ssm map (dummy)
			gnd::bmp8_t				bmp8;
			char path[256];
	...
	if( !::is_proc_shutoff() ){ // ---> operation
		...
		while( !::is_proc_shutoff() ) {
			...
			if( timer_operate.clock() > 0 && ssm_sokuikiraw.readNew()) {
				...
				for( int i = 0 ; i <  ssm_particles.data.size(); i++ ) {
