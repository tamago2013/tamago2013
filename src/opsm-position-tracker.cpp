...
int main(int argc, char* argv[]) {
	...
	if(!::is_proc_shutoff() ){
		...
		gnd::Time::IntervalTimer timer_operate;			// operation timer
		...
		if( !(*pconf.smmapdir.value) ){ // ---> map initialization
			...
			if( !pconf.ndt.value ){
				if( gnd::opsm::build_map(&smmap, &cnt_smmap, gnd_mm2dist(1)) < 0 ){
					...
					::fprintf(stderr, "\n... \x1b[1mOK\x1b[0m success to build opsm map\n");
				}
			}
		...
		while ( !::is_proc_shutoff() ) {
			...
			if( timer_operate.clock() && ssm_sokuikiraw.readNew()  ) {
				...
				optimizer->set_starting_value( starting, ssm_position_read.data.x, ssm_position_read.data.y, ssm_position_read.data.theta );
				...
				{ // ---> 3. optimization iteration by matching laser scanner reading to map(likelihood field)
					...
					if( optimizer->nentry() <= 0 )	continue;
				...
				{ // ---> 6. update map and view data
					...
					{// ---> scanning loop for sokuikiraw-data
						...
						{ // ---> set robot position
							OPSMPosTrack::Viewer::robot_pos.var.x = ssm_position_write.data.x;
							OPSMPosTrack::Viewer::robot_pos.var.y = ssm_position_write.data.y;
							OPSMPosTrack::Viewer::robot_pos.var.t = ssm_position_write.data.theta;
						} // <--- set robot position
